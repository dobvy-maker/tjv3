...
На экране деталей кнопка **Edit Trip** открывает динамический маршрут `edit-trip/[id]`. ID поездки передается через Expo Router. Экран получает текущую поездку через `getTrip(id)`, заполняет поля формы текущими значениями, а после проверки вызывает `updateTrip`. Context обновляет общий массив и сохраняет его в AsyncStorage.

**Что сказать преподавателю:** “Редактирование — это Update из CRUD. Я нахожу запись по ID, создаю новый массив через `map`, заменяю только нужный объект и сохраняю результат в AsyncStorage.”

## Почему поездки сохраняются после перезапуска

`TripsProvider` при старте вызывает `AsyncStorage.getItem`. При add/edit/delete новый массив записывается через `AsyncStorage.setItem`. Дополнительно используется `useRef`, чтобы быстрые последовательные изменения всегда работали с самой свежей версией массива, а не с устаревшим state из предыдущего render.
